...
public class Ex1 {
    public static void main(String[] args) {
        ...
        if (v0 < 0 || angle < 0 || angle > 90) {
            ...
            double distance = (Math.pow(v0, 2) * Math.sin(2 * Math.toRadians(angle))) / 9.81; // konwersja kąta do radianów
            double maxHeight = (Math.pow(v0, 2) * Math.sin(2 * Math.toRadians(angle)) * Math.sin(2 * Math.toRadians(angle)) / (2 * 9.81)); // konwersja kąta do radianów
